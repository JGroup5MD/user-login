...
import org.example.dao.MessageDTO;
...
public class MessageService implements IMessageService{
    ...
    public String getMessages(String userID){
        String s = "<p>Сообщения:</b></p><table><tbody><tr><th>Дата</th><th>От кого</th><th>Кому</th><th>Текст</th></tr>";
        for(MessageDTO message: MessageDAO.getInstance().getAllMessages(userID)){
            s = s + "<tr><td>" + message.getDate() + "</td><td>" + message.getSenderID() + "</td><td>" + message.getReceiverID() + "</td><td>" + message.getText() + "</td></tr>";
        }
        s = s + "</tbody></table>";
        return s;
    }
